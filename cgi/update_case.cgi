...
#
# 次に開く予定のbox のid を受け取る。
#
my $next_box_id = $cgi -> param('next_box_id');
unless(defined($next_box_id) && (length($next_box_id) > 0)){
 $next_box_id = '';
}
...
# T_Case の更新
#
while(my ($_case_id, $ref_case_data) = each(%$case_list)){
 ...
 my $table     = 'T_Case';
 my $condition = "where vcFlowId = '" . $flow_id . "' and vcCaseId = '" . $_case_id . "'";
 $access2db -> set_update(\@set, $table, $condition);
 my $count = $access2db -> update_exe;
}
